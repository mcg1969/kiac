...
func TestK3sRunOptsCarryKernel(t *testing.T) {
	...
	dns := nodeDNSConfig{Servers: []string{"192.168.64.1", "1.1.1.1"}, Options: []string{"timeout:2"}}
	...
	if strings.Join(server.DNS, ",") != "192.168.64.1,1.1.1.1" || strings.Join(server.DNSOptions, ",") != "timeout:2" {
		t.Errorf("server DNS = %q %q", server.DNS, server.DNSOptions)
	}
