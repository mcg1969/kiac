...
type Config struct {
	...
	DNS           []string // node VM nameservers; empty = gateway + public fallbacks (see nodeDNS)
	NoMetrics     bool
	NoStorage     bool
	NoLB          bool
	NoEdgeProxy   bool
...
func (m *Manager) Create(cfg Config) error {
	...
	if err := ui.Step(fmt.Sprintf("Booting %d node VM(s)", len(nodes)), func() error {
		for _, n := range nodes {
			...
			if err := m.rt.RunDetached(runtime.RunOpts{Name: n, Image: cfg.Image, CPUs: cfg.CPUs, Memory: mem, Kernel: cfg.Kernel, DNS: dns.Servers, DNSOptions: dns.Options}); err != nil {
				return err
			}
		}
