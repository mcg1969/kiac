...
func init() {
	...
	f.StringSliceVar(&createCfg.DNS, "dns", nil, "nameserver IPs for the node VMs (default: the network gateway, then this Mac's own DNS servers, then a public resolver as a guaranteed last resort -- so image pulls survive VPN software that takes over the host's port 53)")
