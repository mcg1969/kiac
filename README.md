...
| `--dns` | gateway + `1.1.1.1`,`8.8.8.8` | nameserver IPs for the node VMs. The default keeps the vmnet gateway first and adds public fallbacks so image pulls survive VPN clients that take over the host's port 53 and break the gateway resolver |
